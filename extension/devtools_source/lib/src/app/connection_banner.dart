...
class ConnectionBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Observer(() {
      ...
      final decodeFailures = controller.decodeFailureCount;

      if (state == DevToolsConnectionState.connected &&
          !needsResync &&
          droppedCore == 0 &&
          decodeFailures == 0) {
      ...
      final (IconData icon, String text, Color color) = switch (state) {
        ...
        DevToolsConnectionState.connected => (
          Icons.warning_amber,
          decodeFailures > 0
              ? '$decodeFailures live batch decode failure(s) occurred. '
                    'State was resynchronized; inspect diagnostics.'
              : droppedCore > 0
