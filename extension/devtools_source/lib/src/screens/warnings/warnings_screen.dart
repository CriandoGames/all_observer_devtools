...
class _WarningsScreenState extends State<WarningsScreen> {
  ...
  Widget build(BuildContext context) {
    return Observer(() {
    final List<WarningModel> filtered = widget.store.warnings.where((warning) {
      if (_severityFilter != null && warning.severity != _severityFilter) {
        return false;
      }
      if (_search.isNotEmpty &&
          !warning.message.toLowerCase().contains(_search.toLowerCase()) &&
          !warning.warningCode.toLowerCase().contains(_search.toLowerCase())) {
        return false;
      }
      return true;
              ...
              DropdownButton<WarningSeverityModel?>(
                ...
                items: [
                  ...
                  for (final severity in WarningSeverityModel.values)
                    DropdownMenuItem(value: severity, child: Text(severity.name)),
                ],
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                        [
                          warning.warningCode,
                          if (warning.objectId != null) 'node #${warning.objectId}',
