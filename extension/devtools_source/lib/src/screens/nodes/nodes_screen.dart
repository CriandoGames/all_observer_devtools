...
class _NodesScreenState extends State<NodesScreen> {
  ...
  Widget build(BuildContext context) {
    return Observer(() {
      ...
      final List<NodeModel> filtered = allNodes.where((node) {
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        ...
                        onTap: () =>
                            setState(() => _selectedObjectId = node.objectId),
                      );
                    },
                  ),
                ),
    ...
    return Observer(() {
      ...
      return ListView(
        ...
        children: [
          ...
          if (node.updatedAtMicros != null)
            Text('Last updated (µs): ${node.updatedAtMicros}'),
          if (node.isDisposed) ...[
            const Divider(),
            Text('Disposed at (µs): ${node.disposedAtMicros}'),
            Text(
              'Listener count at dispose: ${node.listenerCountAtDispose ?? '—'}',
            ),
            if (node.disposeReason != null)
              Text('Dispose reason: ${node.disposeReason}'),
          ],
          const Divider(),
          Text(
