  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        for (final scope in scopes)
          Card(
            child: ExpansionTile(
              ...
              subtitle: Text('${scope.resources.length} resource(s) registered'),
              children: [
                for (final resource in scope.resources)
                  ListTile(
                    dense: true,
                    leading: const Icon(Icons.link, size: 16),
                    title: Text('#${resource.resourceId} — ${resource.resourceKind}'),
                  ),
        ...
        if (store.diagnostics.any((d) => d.code == 'scope_dispose_failures')) ...[
          const Divider(),
          const Padding(
            padding: EdgeInsets.all(8),
            child: Text('Disposal diagnostics:'),
          ),
          for (final diag in store.diagnostics.where((d) => d.code == 'scope_dispose_failures'))
            ListTile(
              dense: true,
