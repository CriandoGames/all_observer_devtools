...
void main() {
  group('snapshot followed by events (section 12/31)', () {
    test(
      'applies only events with sequenceNumber > snapshot.lastSequenceNumber',
      () {
        final store = DevToolsStore();
        addTearDown(store.dispose);
        store.applySnapshot(
          _emptySnapshot(sessionId: 's1', lastSequenceNumber: 100),
        );
  ...
  group('sequence gap (section 12/17/31)', () {
    ...
    test(
      'further events are ignored once needsResync is set, until a new snapshot arrives',
      () {
        final store = DevToolsStore();
        addTearDown(store.dispose);
        store.applySnapshot(
        ...
        store.applyEvents([
          _created(sessionId: 's1', sequenceNumber: 106, objectId: 2),
        ]);
  ...
  group('session isolation (section 17/19/31)', () {
    test('events from a foreign session are never applied', () {
      ...
      addTearDown(store.dispose);
      store.applySnapshot(
        _emptySnapshot(sessionId: 'session-A', lastSequenceNumber: 10),
      );
    ...
    test(
      'a new snapshot for a different session replaces state and archives the timeline',
      () {
        final store = DevToolsStore();
        addTearDown(store.dispose);
        store.applySnapshot(
        ...
        store.applyEvents([
          _created(sessionId: 'session-A', sequenceNumber: 11, objectId: 1),
        ]);
        expect(store.timeline, hasLength(1));

        store.applySnapshot(
          _emptySnapshot(sessionId: 'session-B', lastSequenceNumber: 0),
        );
  ...
  group('duplicate events', () {
    test(
      'an event already represented by the snapshot is not applied twice',
      () {
        final store = DevToolsStore();
        addTearDown(store.dispose);
        store.applySnapshot(
          _emptySnapshot(sessionId: 's1', lastSequenceNumber: 5),
        );
  ...
  group('unknown node references (section 31)', () {
    test(
      'update for a node the store never saw flags needsResync, not a crash',
      () {
        final store = DevToolsStore();
        addTearDown(store.dispose);
        store.applySnapshot(
          _emptySnapshot(sessionId: 's1', lastSequenceNumber: 0),
        );
  ...
  group('dependency edges on dispose', () {
    test(
      'disposing a node removes it from every dependency set (mirrors core registry)',
      () {
        final store = DevToolsStore();
        addTearDown(store.dispose);
        store.applySnapshot(
        ...
        store.applyEvents([
          _disposed(sessionId: 's1', sequenceNumber: 2, objectId: 1),
        ]);
  ...
  group('clear()', () {
    test('discards the session entirely, not just the events', () {
      ...
      addTearDown(store.dispose);
      store.applySnapshot(
        _emptySnapshot(sessionId: 's1', lastSequenceNumber: 10),
      );
